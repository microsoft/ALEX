...
std::map<std::string, std::string> parse_flags(int argc, char **argv) {
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (dash != 0) {
            std::cout << "Bad flag '" << argv[i] << "'. Expected --key=value" << std::endl;
            continue;
        }
...
std::string get_required(const std::map<std::string, std::string>& m, const std::string& key) {
    auto it = m.find(key);
    if (it == m.end()) {
...
bool get_boolean_flag(const std::map<std::string, std::string>& m, const std::string& key) {

...
int main(int argc, char* argv[]) {
    ...
    auto payloads = new PAYLOAD_TYPE[init_num_keys];
    ...
    index.bulk_load(keys, payloads, init_num_keys);
    ...
    while (true) {
        ...
        double batch_lookup_time = std::chrono::duration_cast<std::chrono::nanoseconds>(
        ...
        if (print_batch_stats) {
            ...
            std::cout << "Batch " << batch_no << ", cumulative ops: " << cumulative_operations
                      << "\n\tbatch throughput:\t"
                      << num_lookups_per_batch / batch_lookup_time * 1e9 << " lookups/sec,\t"
                      << num_actual_inserts / batch_insert_time * 1e9 << " inserts/sec,\t"
    ...
    std::cout << "Cumulative stats: " << batch_no << " batches, "
              ...
              << cumulative_lookups << " lookups, " << cumulative_inserts << " inserts)"
              << "\n\tcumulative throughput:\t"
              << cumulative_lookups / cumulative_lookup_time * 1e9 << " lookups/sec,\t"
              << cumulative_inserts / cumulative_insert_time * 1e9 << " inserts/sec,\t"
              << cumulative_operations / cumulative_time * 1e9 << " ops/sec"

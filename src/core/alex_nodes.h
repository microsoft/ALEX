 ...
 * - Data nodes, sometimes referred to as leaf nodes (equivalent to leaf nodes of a B+ Tree)
 */
...
public:
    ...
    AlexModelNode(const AlexModelNode& other) : AlexNode<T,P>(other), num_children_(other.num_children_) {
    ...
    inline AlexNode<T,P>* get_child_node(T key) {
    ...
    bool validate_structure(bool verbose=false) const {
        if (num_children_ == 0) {
            if (verbose) {
                std::cout << "[Childless node] addr: " << this
                          << ", level " << this->level_
                          << std::endl;
            }
            return false;
        ...
        if (num_children_ == 1) {
            if (verbose) {
                std::cout << "[Single child node] addr: " << this
                          << ", level " << this->level_
                          << std::endl;
            }
            return false;
        }
        if (std::ceil(std::log2(num_children_)) != std::floor(std::log2(num_children_))) {
            if (verbose) {
                std::cout << "[Num children not a power of 2] num children: " << num_children_ << std::endl;
            }
            return false;
        }
        ...
        if (this->model_.a_ == 0) {
            if (verbose) {
                std::cout << "[Model node with zero slope] addr: " << this
                          << ", level " << this->level_
                          << std::endl;
            }
            return false;
        ...
        for (i = 1; i < num_children_; i++) {
            if (children_[i] == cur_child) {
                ...
                if (cur_repeats != (1 << cur_child->duplication_factor_)) {
                    if (verbose) {
                        std::cout << "[Incorrect duplication factor] num actual repeats: " << cur_repeats
                                  << ", num dup_factor repeats: " << (1 << cur_child->duplication_factor_)
                                  << ", parent addr: " << this
                ...
                if (std::ceil(std::log2(cur_repeats)) != std::floor(std::log2(cur_repeats))) {
                    if (verbose) {
                        std::cout << "[Num duplicates not a power of 2] num actual repeats: " << cur_repeats
                                  << std::endl;
                    }
                    return false;
                }
                if (i % cur_repeats != 0) {
                    if (verbose) {
                        std::cout << "[Duplicate region incorrectly aligned] num actual repeats: " << cur_repeats
                                  << ", num dup_factor repeats: " << (1 << cur_child->duplication_factor_)
                                  << ", child pointer indexes: [" << i - cur_repeats << ", " << i << ")"
                                  << std::endl;
                    }
        ...
        if (std::ceil(std::log2(cur_repeats)) != std::floor(std::log2(cur_repeats))) {
            if (verbose) {
                std::cout << "[Num duplicates not a power of 2] num actual repeats: " << cur_repeats << std::endl;
            }
            return false;
        }
        if (i % cur_repeats != 0) {
        ...
        if (cur_repeats == num_children_) {
            if (verbose) {
                std::cout << "[All children are the same] num actual repeats: " << cur_repeats
                          << ", parent addr: " << this
                          << ", parent level: " << this->level_
                          << ", parent num children: " << num_children_
                          << std::endl;
...
public:
    ...
    T min_key_ = std::numeric_limits<T>::max();  // min key in node, updates after inserts but not erases
    ...
    static void build_node_implicit(const T* keys, int num_keys, int data_capacity, StatAccumulator* acc,
        ...
        {
            int predicted_position = std::max(0, std::min(data_capacity - 1, model->predict(keys[i])));
            ...
            if (positions_remaining < keys_remaining) {
                ...
                for (int j = i; j < num_keys; j++) {
                    predicted_position = std::max(0, std::min(data_capacity - 1, model->predict(keys[j])));
    ...
    static double compute_expected_cost_sampling(const T* keys, int num_keys, double density, double expected_insert_frac,
        ...
        if (num_keys < exact_computation_size_threshold) {
            return compute_expected_cost(keys, num_keys, density, expected_insert_frac, existing_model, false, stats);
        }
        ...
        while (true) {
            ...
            if (sample_stats.size() >= 3) {
                ...
                // (y1 - y0) / (x1 - x0) = (y2 - y1) / (x2 - x1) --> y2 = (y1 - y0) / (x1 - x0) * (x2 - x1) + y1
                double expected_s2_search_iters = (s1.num_search_iterations - s0.num_search_iterations) / (s1.log2_sample_size - s0.log2_sample_size) * (s2.log2_sample_size - s1.log2_sample_size) + s1.num_search_iterations;
                double rel_diff = std::abs((s2.num_search_iterations - expected_s2_search_iters) / s2.num_search_iterations);
                if (rel_diff <= rel_diff_threshold || num_keys <= 2 * sample_num_keys) {
                    search_iters_computed = true;
                    expected_full_search_iters = (s2.num_search_iterations - s1.num_search_iterations) / (s2.log2_sample_size - s1.log2_sample_size) * (log2_num_keys - s2.log2_sample_size) + s2.num_search_iterations;
                }
                if (compute_shifts) {
                    double expected_s2_log2_shifts = (s1.log2_num_shifts - s0.log2_num_shifts) / (s1.log2_sample_size - s0.log2_sample_size) * (s2.log2_sample_size - s1.log2_sample_size) + s1.log2_num_shifts;
                    double abs_diff = std::abs((s2.log2_num_shifts - expected_s2_log2_shifts) / s2.log2_num_shifts);
                    if (abs_diff <= abs_log2_diff_threshold || num_keys <= 2 * sample_num_keys) {
                        shifts_computed = true;
                        double expected_full_log2_shifts = (s2.log2_num_shifts - s1.log2_num_shifts) / (s2.log2_sample_size - s1.log2_sample_size) * (log2_num_keys - s2.log2_sample_size) + s2.log2_num_shifts;
                        expected_full_shifts = std::pow(2, expected_full_log2_shifts);
                    }
                }

                // If diff in stats is sufficiently small, return the approximate expected cost
                if ((expected_insert_frac == 0 && search_iters_computed) ||
                    (expected_insert_frac > 0 && search_iters_computed && shifts_computed)) {
                    double cost = kExpSearchIterationsWeight * expected_full_search_iters +
    ...
    static void build_node_implicit_sampling(const T* keys, int num_keys, int sample_num_keys,
        ...
        {
            int predicted_position = std::max(0, std::min(sample_data_capacity - 1, sample_model->predict(keys[i])));
            int actual_position = std::max<int>(predicted_position, last_position + 1);
            ...
            if (positions_remaining < sample_keys_remaining) {
                ...
                for (int j = i; j < num_keys; j += step_size) {
                    predicted_position = std::max(0, std::min(sample_data_capacity - 1, sample_model->predict(keys[j])));
    ...
    static void build_model_sampling(const T* keys, int num_keys, LinearModel<T>* model, bool verbose = false) {
        ...
        if (verbose) {
            std::cout << "Build index, sample size: " << num_keys / step_size << " (a, b): (" << prev_a << ", " << prev_b << ")" << std::endl;
        }

        // Keep decreasing step size (increasing sample size) until model does not change significantly
        while (step_size > 1) {
            step_size /= sample_size_multiplier;
            ...
            if (verbose) {
                std::cout << "Build index, sample size: " << num_keys / step_size << " (a, b): (" << model->a_ << ", " << model->b_ << ") ("
                          << rel_change_in_a << ", " << rel_change_in_b << ")" << std::endl;
            }
    ...
    inline int predict_position(T key) const {
    ...
    int find_insert_position(T key) {
        ...
        if (predicted_pos <= pos) {
            ...
            // Place inserted key as close as possible to the predicted position while maintaining correctness
    ...
    // Returns position in range [0, data_capacity]
    template <class K>
    inline int exponential_search_upper_bound(int m, K key) {
        // Continue doubling the bound until it contains the upper bound. Then use binary search.
        int bound = 1;
    ...
    // Returns position in range [0, data_capacity]
    template <class K>
    inline int exponential_search_lower_bound(int m, K key) {
    ...
    int closest_gap(int pos) const {
        ...
        if (bitmap_[bitmap_pos] == static_cast<uint64_t>(-1) || (
            ...
            // no gaps in this block of 64 positions, start searching in adjacent blocks
...
int closest_gap(int pos) const {
    ...
    int erase_one(T key) {
        ...
        if (pos == data_capacity_ || ALEX_DATA_NODE_KEY_AT(pos) != key)
            return 0;
    ...
    bool validate_structure(bool verbose=false) const {
        ...
        for (int i = 0; i < data_capacity_ - 1; i++) {
            if (ALEX_DATA_NODE_KEY_AT(i) > ALEX_DATA_NODE_KEY_AT(i+1)) {
                ...
                return false;
            } else if (ALEX_DATA_NODE_KEY_AT(i) < ALEX_DATA_NODE_KEY_AT(i+1) && !check_exists(i)) {
                if (verbose) {
                    std::cout << "The last key of a certain value should not be a gap" << std::endl;
                }
                return false;
            }
        }
        if (ALEX_DATA_NODE_KEY_AT(data_capacity_ - 1) == kEndSentinel_ && check_exists(data_capacity_ - 1)) {
            if (verbose) {
        ...
        if (num_bitmap_ones != num_keys_) {
            if (verbose) {
                std::cout << "Number of ones in bitmap should match num_keys" << std::endl;
            }
            return false;
        }
        return true;
    ...
    // If validate_bitmap is true, confirm that the corresponding position in the bitmap is correctly set to 1
    bool key_exists(T key, bool validate_bitmap) const {
        for (int i = 0; i < data_capacity_ - 1; i++) {
            if (ALEX_DATA_NODE_KEY_AT(i) == key && (!validate_bitmap || check_exists(i))) {
                return true;
            }
        }
        return false;
    ...
    std::string to_string() const {
        std::string str;
        str += "Num keys: " + std::to_string(num_keys_) + ", Capacity: " + std::to_string(data_capacity_) + ", Expansion Threshold: " + std::to_string(expansion_threshold_) + "\n";
        for (int i = 0; i < data_capacity_; i++) {

...
struct FTNode {
    ...
    int right_boundary;  // end position (exclusive) in input array that this node represents
    bool use = false;
...
void collect_used_nodes(std::vector<std::vector<FTNode>>& fanout_tree, int max_level, std::vector<FTNode>& used_fanout_tree_nodes) {
    max_level = std::min(max_level, static_cast<int>(fanout_tree.size()) - 1);
    for (int i = 0; i <= max_level; i++) {
        auto& level = fanout_tree[i];
        for (FTNode& tree_node : level) {
            if (tree_node.use) {
                used_fanout_tree_nodes.push_back(tree_node);
            }
        }
    }
    std::sort(used_fanout_tree_nodes.begin(), used_fanout_tree_nodes.end(), [&](FTNode& left, FTNode& right){
        // this is better than comparing boundary locations
        return (left.node_id << (max_level - left.level)) < (right.node_id << (max_level - right.level));
    });
}

// Starting from a complete fanout tree of a certain depth, merge tree nodes upwards if doing so decreases the cost.
// Returns the new best cost.
// This is a helper function for finding the best fanout in a bottom-up fashion.
template <class T, class P>
static double merge_nodes_upwards(int start_level, double best_cost, int num_keys, int total_keys,
    ...
    for (int level = start_level; level >= 1; level--) {
        ...
        for (int i = 0; i < level_fanout / 2; i++) {
            if (fanout_tree[level][2 * i].use && fanout_tree[level][2 * i + 1].use) {
                ...
                double merging_cost_saving = (fanout_tree[level][2 * i].cost * num_left_keys / num_node_keys) +
...
// For example, level 3 will have 8 tree nodes, which are returned through used_fanout_tree_nodes.
// Assumes node has already been trained to produce a CDF value in the range [0, 1).
template <class T, class P>
double compute_level(T keys[], int num_keys, AlexNode<T,P>* node,
                                     int total_keys, std::vector<FTNode>& used_fanout_tree_nodes,
                                     int level, double expected_insert_frac = 0,
                     bool approximate_model_computation = true, bool approximate_cost_computation = false) {
    int fanout = 1 << level;
    double cost = 0.0;
    ...
    for (int i = 0; i < fanout; i++) {
        ...
        right_boundary = i == fanout - 1 ? num_keys : static_cast<int>(std::lower_bound(keys, keys + num_keys,
                                                                       ((i + 1) - b) /
                                                                       a) - keys);
        if (left_boundary == right_boundary) {
            used_fanout_tree_nodes.push_back({ level, i, 0, left_boundary, right_boundary, true, 0, 0, 0, 0, 0 });
            continue;
        }
        LinearModel<T> model;
        AlexDataNode<T,P>::build_model(keys + left_boundary, right_boundary - left_boundary, &model,
                                        approximate_model_computation);

        DataNodeStats stats;
        double node_cost = AlexDataNode<T,P>::compute_expected_cost(
                keys + left_boundary, right_boundary - left_boundary, AlexDataNode<T,P>::kInitDensity_,
                expected_insert_frac, &model, approximate_cost_computation, &stats);
    ...
    double traversal_cost = kNodeLookupsWeight +
                            (kModelSizeWeight * fanout * (sizeof(AlexDataNode<T,P>) + sizeof(void*)) * total_keys / num_keys);
    cost += traversal_cost;
    return cost;
}

// Figures out the optimal partitioning of children in a "bottom-up" fashion (see paper for details).
// Assumes node has already been trained to produce a CDF value in the range [0, 1).
// Returns the depth of the best fanout tree and the total cost of the fanout tree.
template <class T, class P>
std::pair<int, double> find_best_fanout_bottom_up(T keys[], int num_keys, AlexNode<T,P>* node,
                                                  int total_keys, std::vector<FTNode>& used_fanout_tree_nodes,
                                                  int max_fanout, double expected_insert_frac = 0,
                                                  bool approximate_model_computation = true, bool approximate_cost_computation = false) {
    // Repeatedly add levels to the fanout tree until the overall cost of each level starts to increase
    int best_level = 0;
    double best_cost = node->cost_ + kNodeLookupsWeight;
    std::vector<double> fanout_costs;
    std::vector<std::vector<FTNode>> fanout_tree;
    fanout_costs.push_back(best_cost);
    fanout_tree.push_back({ { 0, 0, best_cost, 0, num_keys, false, 0, 0, 0, 0, num_keys } });
    for (int fanout = 2, fanout_tree_level = 1; fanout <= max_fanout; fanout *= 2, fanout_tree_level++) {
        std::vector<FTNode> new_level;
        double cost = compute_level(keys, num_keys, node, total_keys, new_level, fanout_tree_level, expected_insert_frac,
...
// Returns the depth of the best fanout tree and the total cost of the fanout tree.
template <class T, class P>
std::pair<int, double> find_best_fanout_top_down(T keys[], int num_keys, AlexNode<T,P>* node,
                                                 int total_keys, std::vector<FTNode>& used_fanout_tree_nodes,
                                                 int max_fanout, double expected_insert_frac = 0,
                                                 bool approximate_model_computation = true, bool approximate_cost_computation = false) {
    // Grow the fanout tree top-down breadth-first, each node independently instead of complete levels at a time
    std::vector<std::vector<FTNode>> fanout_tree;
    ...
    while (true) {
        ...
        for (FTNode& tree_node : fanout_tree[fanout_tree_level-1]) {
            ...
            int middle_boundary = static_cast<int>(std::lower_bound(keys + tree_node.left_boundary, keys + tree_node.right_boundary,
                                                   ((2 * tree_node.node_id + 1) - b) /
                                                   a) - keys);
            double node_split_cost = 0;
            int num_node_keys = tree_node.right_boundary - tree_node.left_boundary;
            int boundaries[] = { tree_node.left_boundary, middle_boundary, tree_node.right_boundary };
            double node_costs[2];
            ...
            for (int i = 0; i < 2; i++) {
                ...
                AlexDataNode<T,P>::build_model(keys + left, right - left, &node_models[i],
                                                approximate_model_computation);
                node_costs[i] = AlexDataNode<T,P>::compute_expected_cost(
                        keys + left, right - left, AlexDataNode<T,P>::kInitDensity_,
                        expected_insert_frac, &node_models[i], approximate_cost_computation, &node_stats[i]);
            }
            node_split_cost += sizeof(AlexDataNode<T,P>) * kModelSizeWeight * total_keys / num_node_keys;
            if (node_split_cost < tree_node.cost) {
                cost_savings_from_level += (tree_node.cost - node_split_cost) * num_node_keys / num_keys;
                for (int i = 0; i < 2; i++) {
                    new_level.push_back({
                                                fanout_tree_level,
                                                2 * tree_node.node_id + i,
                                                node_costs[i],
                                                boundaries[i],
                                                boundaries[i+1],
                                                true,
                                                node_stats[i].num_search_iterations,
                                                node_stats[i].num_shifts,
                                                node_models[i].a_,
                                                node_models[i].b_,
        ...
        double level_cost = kModelSizeWeight * sizeof(void*) * fanout / 2 * total_keys / num_keys;  // cost of 2X pointers
...
// This mirrors the logic of finding the best fanout "bottom-up" when bulk loading.
// Returns the depth of the best fanout tree.
template <class T, class P>
int find_best_fanout_existing_node(
        AlexModelNode<T,P>* parent, int bucketID, int total_keys, std::vector<FTNode> &used_fanout_tree_nodes,
        int max_fanout) {
    // Repeatedly add levels to the fanout tree until the overall cost of each level starts to increase
    ...
    for (int fanout = 1, fanout_tree_level = 0; fanout <= max_fanout; fanout *= 2, fanout_tree_level++) {
        std::vector<FTNode> new_level;
        double cost = 0.0;
        ...
        for (int i = 0; i < fanout; i++) {
            left_boundary = right_boundary;
            right_boundary = i == fanout - 1 ? node->data_capacity_ : node->lower_bound(
                    ((i + 1) - b) /
                    a);
            if (left_boundary == right_boundary) {
                new_level.push_back({ fanout_tree_level, i, 0, left_boundary, right_boundary, false, 0, 0, 0, 0, 0 });
                continue;
            }
            int num_actual_keys = 0;
            LinearModel<T> model;
            typename AlexDataNode<T,P>::Iterator it(node, left_boundary);
            ...
            DataNodeStats stats;
            double node_cost = AlexDataNode<T,P>::compute_expected_cost_from_existing(
                    node, left_boundary, right_boundary, AlexDataNode<T,P>::kInitDensity_,
